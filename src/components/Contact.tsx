...
const uaeOffices = [
  {
    ...
    details: [
      ...
      { icon: <PhoneIcon className="w-[18px] h-[18px]" />, text: "+971 505056093" },
      { icon: <MailIcon className="w-[19px] h-[19px]" />, text: "sales@rockdoveaiation.com" },
    ],
  },
];
      ...
      <section className="grid grid-cols-2 gap-24 px-24 mb-32">
          ...
          {uaeOffices.map((office, idx) => (
            <div key={idx} className="space-y-8">
              <div className="flex items-center gap-2">
                {office.icon}
                <h2 className="text-white text-xl font-medium">{office.title}</h2>
              </div>
              {office.details.map((detail, i) => (
                <div key={i} className="flex items-start gap-2">
                  {detail.icon}
                  <p className="text-white text-xl font-medium">{detail.text}</p>
                </div>
              ))}
            </div>
          ))}
        ...
        <div className="space-y-12">
          ...
          <div className="space-y-8">
            <div className="flex items-center gap-2">
              {indiaOffice.icon}
              <h2 className="text-white text-xl font-medium">{indiaOffice.title}</h2>
            </div>
            {indiaOffice.details.map((detail, i) => (
              <div key={i} className="flex items-start gap-2">
                {detail.icon}
                {detail.text && <p className="text-white text-xl font-medium">{detail.text}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-24 mb-36">
        <div className="text-center mb-24">
          <h2 className="text-white text-2xl font-bold">
            ...
            <br />
            A representative from our company will respond soon.
          </h2>
        </div>
        ...
        <form className="grid gap-6 max-w-xl mx-auto">
          {formFields.map((field) => (
            <div key={field.id} className="space-y-2">
              <label htmlFor={field.id} className="text-[#5cc6d0] font-semibold text-xl">
                {field.label}
              </label>
              <input
                id={field.id}
